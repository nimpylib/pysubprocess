...
when defined(js):
  import std/jsffi
  from pkg/jscompat/utils/denoAttrs import
    collectImportNode, importByNodeOrDeno, importNode

  {.emit: """/*INCLUDESECTION*/
  function pysubprocessOptions(input, cwd, shell, envPairs) {
    const options = {
      encoding: 'utf8',
      input: input,
    ...
    if (envPairs !== null) {
      ...
      for (let i = 0; i < envPairs.length; i += 2)
        options.env[envPairs[i]] = envPairs[i + 1];
    }
    return options;
  }
  ...
  proc subprocessOptions(input, cwd: cstring; shell: bool;
      envPairs: JsObject): JsObject
      {.importjs: "pysubprocessOptions(#, #, #, #)".}
  proc spawnSync(command: cstring; args: seq[cstring];
      options: JsObject): JsRunResult
      {.importNode(child_process, spawnSync).}
  collectImportNode()
...
proc runImpl(command: string; commandArgs: openArray[string]; input: string;
  ...
  when defined(js):
    ...
    let options = subprocessOptions(cstring(input), cstring(cwd), shell, jsEnv)
    let child = spawnSync(cstring(command), jsArgs, options)
